...
public class FlourideInfuser extends Block{
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		...
		return Item.getItemFromBlock(Modblocks.FLOURIDE_INFUSER);
	}
...
public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
	...
	public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
	 ...
	 public static void SetState(boolean act, World worldIn, BlockPos pos)
	 {
		 ...
		 if(act) worldIn.setBlockState(pos, Modblocks.FLOURIDE_INFUSER.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(INFUSING, true), 3);
		 else worldIn.setBlockState(pos, Modblocks.FLOURIDE_INFUSER.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(INFUSING, false), 3);
		 if(tileentity != null)
		 {
			 tileentity.validate();
