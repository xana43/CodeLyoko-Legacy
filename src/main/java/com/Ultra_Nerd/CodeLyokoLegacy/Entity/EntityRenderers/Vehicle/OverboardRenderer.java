...
public final class OverboardRenderer<T extends EntityRenderState> extends EntityRenderer<OverboardEntity,T> {


    private static final Identifier OVERBOARD_TEX = CodeLyokoMain.codeLyokoPrefix(
            "textures/entity/overboard/overboarduv.png");
    //private final @NotNull EntityModel<EntityRenderState> OverBoardModel;

    public OverboardRenderer(EntityRendererFactory.@NotNull Context renderManager) {
        super(renderManager);
        this.shadowRadius = 1f;
//        this.OverBoardModel = new ModelOverboard<T>(renderManager.getPart(ModelOverboard.LAYER_LOCATION));
