...
public final class SuperComputerCoreBlockEntity extends EnergyStorageBlockEntity implements GeoBlockEntity {
    private final SingleVariantStorage<FluidVariant> core_tank_chilled =
            MethodUtil.FluidStorageCreation.createFluidStorage(this, Fluids.WATER,new NbtCompound());
    ...
    public SuperComputerCoreBlockEntity(final BlockPos pos, final BlockState state) {
        super(ModBlockEntities.SUPERCOMPUTER_CORE_BLOCK_ENTITY_TYPE, pos, state,90000000,
                10000,0);
    }
    ...
    public void readNbt(final NbtCompound nbt,final RegistryWrapper.WrapperLookup registryLookup) {
        ...
        core_tank_chilled.amount = nbt.getLong("chilled_amount").orElse(0L);
        core_tank_hot.amount = nbt.getLong("hot_amount").orElse(0L);
        ...
        poweredOn = nbt.getBoolean("is_powered_on").orElse(false);
    }
