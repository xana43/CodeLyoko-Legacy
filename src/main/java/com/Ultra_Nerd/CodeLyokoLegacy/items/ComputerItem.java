...
public final class ComputerItem extends Item {
    ...
    public void appendTooltip(final ItemStack stack, @Nullable final World world, final List<Text> tooltip, final TooltipContext context) {
        ...
        {
            tooltip.add(new TranslatableTextContent("computer.item.tooltip.shift").setStyle(ConstantUtil.Styles.HUD.getThisStyle()));
        }
        else
        {
            tooltip.add(new TranslatableTextContent("computer.item.tooltip"));
