...
public record PacketHandler() {
    ...
    private static void commonChannelRegistry() {
        //ServerPlayNetworking.registerGlobalReceiver(ControlPanelPayload.ID, AuxiliaryControlPanelActivationC2SPacket::receive);
        //ServerPlayNetworking.registerGlobalReceiver(TowerPayload.ID, TowerActivationC2SPacket::receive);
        ...
        //ServerPlayNetworking.registerGlobalReceiver(ClassScreenPayload.ID, UpdatePlayerClassC2SPacket::receive);
...
//        ServerPlayNetworking.registerGlobalReceiver(NinjaPayloads.NinjaPayloadBlock.ID,NinjaRayCastC2SPacket::receive);
    ...
    public static void commonPayloadRegistry(){
        PayloadTypeRegistry.playS2C().register(ClassScreenPayload.ID,ClassScreenPayload.CODEC);
        PayloadTypeRegistry.playS2C().register(ControlPanelPayload.ID,ControlPanelPayload.CODEC);
        PayloadTypeRegistry.playS2C().register(TowerPayload.ID,TowerPayload.CODEC);
        PayloadTypeRegistry.playS2C().register(NinjaPayloads.NinjaPayloadBlock.ID,NinjaPayloads.NinjaPayloadBlock.CODEC);
        commonChannelRegistry();
    }
    @Environment(EnvType.CLIENT)
    public static void clientPayloadRegistry(){
        PayloadTypeRegistry.playC2S().register(ClassScreenPayload.ID,ClassScreenPayload.CODEC);
        PayloadTypeRegistry.playC2S().register(ControlPanelPayload.ID,ControlPanelPayload.CODEC);
        clientPacketRegistry();
    }
    @Environment(EnvType.CLIENT)
    private static void clientPacketRegistry() {
       /* ClientPlayNetworking.registerGlobalReceiver(BUILD_HOLOGRAM,RenderBuildAreaS2CPacket::receive);
        ...
        ClientPlayNetworking.registerGlobalReceiver(ClassScreenPayload.ID, ClassScreenS2CPacket::receive);
