...
public class LyokoChunkGenerator extends NoiseChunkGenerator<LyokoGenSettings> {
    ...
    protected double[] getBiomeNoiseColumn(int noiseX, int noiseZ) {
        ...
        float f3 = this.biomeProvider.func_225526_b_(noiseX, j, noiseZ).getDepth();

        for (int k = -2; k <= 2; ++k) {
            for (int l = -2; l <= 2; ++l) {
                Biome biome = this.biomeProvider.func_225526_b_(noiseX + k, j, noiseZ + l);
    ...
    private double getNoiseDepthAt(int noiseX, int noiseZ) {
        double d0 = this.depthNoise.getValue((double) (noiseX * 200),
                10.0D, (double) (noiseZ * 200), 1.0D,
                0.0D, true)
                * 65535.0D / 8000.0D;
        if (d0 < 0.0D) {
            d0 = -d0 * 0.3D;
    ...
    protected void fillNoiseColumn(double[] noiseColumn, int noiseX, int noiseZ) {
        ...
        int i = -10;
        int j = 3;
        this.func_222546_a(noiseColumn, noiseX, noiseZ, (double) 684.412F, (double) 684.412F, 8.555149841308594D,
